...
The container `nickfedor/watchtower`(fork) has been created to automatically update containers when a new image is released.
...
The image has been around since 2015, has several thousand stars, [the (fork) source code is available for public scrutiny](https://github.com/nicholas-fedor/watchtower/) and many people run it \(myself included\), so the risk of nefarious behaviour by the authors is \(in my opinion\) fairly low. Nevertheless, if you decide to give this container access to the host's `/var/run/docker.sock`, you do so at your own risk.
